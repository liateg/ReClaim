...
describe("Claims API", () => {
    ...
    it("lists only the authenticated user's claims", async () => {
        ...
        expect(res.body.claims.some((claim) => claim.id === claimId)).to.equal(true);
        expect(res.body.claims.some((claim) => claim.id === rejectedClaimId)).to.equal(false);
    });
    it("lets admin list all claims", async () => {
        ...
        expect(res.body.claims).to.be.an("array");
        expect(res.body.claims.some((claim) => claim.id === claimId)).to.equal(true);
        expect(res.body.claims.some((claim) => claim.id === rejectedClaimId)).to.equal(true);
    });
