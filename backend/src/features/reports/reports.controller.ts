...
export const createReport = async (req: Request, res: Response) => {
  ...
  const { itemId = null, claimId = null, reason, description = null } = req.body;
  ...
  try {
    ...
    if (itemId === null && claimId === null) {
      return res.status(400).json({ message: "Either itemId or claimId is required" });
    }
    ...
    return res.status(201).json({ message: 'Report created successfully', report: toReportResponse(created.rows[0]) });
  } catch (error) {
...
export const getReports = async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(`SELECT ${reportSelect} FROM reports ${where} ORDER BY id DESC`, params);
...
export const getReportById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(`SELECT ${reportSelect} FROM reports WHERE id = $1`, [id]);
    if (result.rows.length === 0) return res.status(404).json({ message: 'Report not found' });

    const report = result.rows[0] as any;
    if (auth.role !== 'admin' && Number(report.reporterId) !== Number(auth.id)) {
...
export const updateReport = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const existing = await pool.query(`SELECT reporter_id AS "reporterId" FROM reports WHERE id = $1`, [id]);
    ...
    if (!isAdmin && !isOwner) return res.status(403).json({ message: 'You do not have permission to perform this action' });
    ...
    if (fragments.length === 0) return res.status(400).json({ message: 'No updatable fields provided or insufficient permissions' });

    values.push(Number(id));

    const result = await pool.query(`UPDATE reports SET ${fragments.join(', ')} WHERE id = $${values.length} RETURNING ${reportSelect}`, values);

    return res.status(200).json({ message: 'Report updated successfully', report: toReportResponse(result.rows[0]) });
  } catch (error) {
