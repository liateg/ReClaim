...
void main() {
  group('Integration: Items/Claims/Profile -> Cache -> Model -> Backend', () {
    ...
    test('login to backend', () async {
      ...
      // Make services use this token in tests
      AuthService.setTestToken(token);
    });

    test('items: fetch, map, cache, expiry', () async {
      // Fetch items (authenticated)
      print('Requesting /items from backend...');
      final itemsList = await ItemsService()
          .getItems(forceRefresh: true, ttl: Duration(seconds: 2));
      ...
      print(
          'Mapped first item to ItemModel: id=${model.id}, title=${model.title}');
    ...
    test('claims: fetch, map, cache, expiry', () async {
      ...
      final claimsList = await ClaimsService()
          .getClaims(forceRefresh: true, ttl: Duration(seconds: 2));
      ...
      print(
          'Mapped first claim: id=${claimModel.id}, itemId=${claimModel.itemId}, status=${claimModel.status}');
    ...
    test('reports: fetch, map, cache, expiry', () async {
      ...
      final reportsList = await ReportsService()
          .getReports(forceRefresh: true, ttl: Duration(seconds: 2));
    ...
    test('profile: fetch current user and map to ProfileModel', () async {
      ...
      print(
          'Mapped profile: id=${profile.id}, email=${profile.email}, name=${profile.name}');
      ...
      await Future.delayed(Duration(seconds: 3));
      final afterExpiry =
          await cache.get<Map<String, dynamic>>(profileCacheKey);
      expect(afterExpiry, isNull);
